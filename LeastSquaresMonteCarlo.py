...
class LSMC:
	def __init__(self, type, spot, strike, time, interest, dividend, volatility, period, simulations, degree=2):
		self.type = type
	...
	def random_path(self):
		...
		for i in range(1, self.period + 1):
			self.randomWalk[:,i] = self.randomWalk[:,i - 1] * np.exp((self.interest_rate - self.dividend_rate + (self.volatility_rate ** 2) / 2) * self.deltaT + \
